...
async function fetchPosts(page = 1, categoryId = null) {
    ...
    let url = `http://the-groove-grid.local/wp-json/wp/v2/posts?page=${page}&per_page=10`;
    if (categoryId) {
...
async function fetchImages(mediaIds) {
    const requests = mediaIds.map(id => fetch(`http://the-groove-grid.local/wp-json/wp/v2/media/${id}`));
...
async function fetchCategories() {
    try {
        const response = await fetch("http://the-groove-grid.local/wp-json/wp/v2/categories");
        if (!response.ok) {
            throw new Error (`HTTP error! Status: ${response.status}`);
        } 
...
async function fetchAuthors() {
    try {
        const response = await fetch("http://the-groove-grid.local/wp-json/wp/v2/users");
...
async function filterPostsByCategory(categoryId) {
    ...
    try {
        const url = `http://the-groove-grid.local/wp-json/wp/v2/posts?categories=${categoryId}&per_page=10`;
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status}`);
        }
